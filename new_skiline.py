...
class MyFormatter(commands.HelpFormatter):
    async def format(self):
        ...
        if self.is_bot():
            ...
            for category, _commands in itertools.groupby(
                    data, key=category):
                # there simply is no prettier way of doing
                # this.
                _commands = sorted(
                    _commands)
                if len(_commands) > 0:
...
class Normal_Command:
    ...
    async def poll(self, ctx, *args):
        ...
        if 1 <= len(args) <= 21:
            ...
            m = await ctx.send(
                '**{0}**'.format(args[0]), embed=embed)
            [self.client.loop.create_task(
                m.add_reaction(e)) for e in emojis]
...
class Staff_Command:
    ...
    async def __local_check(self, ctx):
        ...
        return (any(x in role_ids for x in
                    (429281099672322056,
                     268352165175623680, )
                    )
                )
    ...
    async def on_ready(self):
        self.limit_role: discord.Role = \
            client.get_guild(235718949625397251).get_role(
                412567728067575809)

    @commands.command(brief='制限付きユーザーを付けます')
    async def limit(self, ctx, member: discord.Member):
        Tasks = [self.client.loop.create_task(member.remove_roles(r))
                 for r in member.roles if r != self.limit_role]
...
class Owners_Command:
    ...
    async def on_ready(self):
        self.index_index = client.get_channel(
            500274844253028353)

    # インデックスチャンネルをサーチ。なければNone
    def _create_category_find_index_channel(
            self, category) -> discord.TextChannel:
        try:
    ...
    # インデックスの下のEmbedを作る方。
    async def _create_category_index2(self, channel):
        index_channel = self._create_category_find_index_channel(
            channel.category)
        if index_channel is not None:
            async for message in (index_channel.history(reverse=True)
                                  .filter(lambda m: m.author == self.client.user and m.embeds)):
                match = self.id_match.search(
                    message.embeds[0].description)
                if match and channel.id == int(
                        match.group(1)):
                    break
            else:
                try:
    ...
    async def on_guild_channel_update(self, before, after):
        if isinstance(
                after,
                discord.TextChannel) and after.name != 'category-index':
            if before.category is not None and (
                    after.category is None or before.category != after.category):
    ...
    async def create_category_index(self, ctx, *args):
        async def _create_category_index(category, ctx=None):
            ...
            if index_channel is None:
                ...
                tasks = [
                    self.client.loop.create_task(
                        self._create_category_index2(channel)) for channel in sorted(
                        (c for c in category.channels if isinstance(
                            c,
                            discord.TextChannel) and c != index_channel),
                        key=lambda c:c.position)]
...
class DM_Command:
    ...
    async def on_message(self, message: discord.Message):
        if isinstance(
                message.channel,
                discord.DMChannel) and message.author in self.users and not message.content.startswith(
...
async def on_member_join(member):
    if 'discord.gg' in member.display_name:
        ...
        channel = next(
            c for c in member.guild.channels if c.name == '雑談フォーラム')
        try:
...
""".format(member.mention, member.guild.name)
        channel = next(
            c for c in member.guild.channels if c.name == 'ニューメンバー')
...
async def on_member_remove(member):
    name = member.display_name
    embed = discord.Embed(
        title='{0}さんが退出しました。'.format(
            name),
        colour=0x2E2EFE,
        description='{0}さん、ご利用ありがとうございました。\nこのサーバーの現在の人数は{1}人です'.format(
            name,
            member.guild.member_count))
    embed.set_thumbnail(
        url=member.avatar_url)
...
async def on_ready():
    ...
    with open(os.path.dirname(__file__) + os.sep + 'config.yaml', encoding='utf-8') as f:
        data = yaml.load(
            f)
    role_ids = data['roles']
    rolelist = [guild.get_role(
        int(i)) for i in role_ids]
    [rolelist.remove(None)
     for i in rolelist[:] if i is None]
    await create_role_panel()
    print(
        client.user.name,
        client.user.id,
...
async def on_voice_state_update(member, before, after):
    ...
    if after.channel is not None and (
            before.channel is None or before.channel != after.channel) and str(
            after.channel.id) in voice_text_pair:
        text_channel = client.get_channel(
            voice_text_pair[str(after.channel.id)])
        embed = discord.Embed(
            title='ボイスチャンネル入室通知',
            description='{0}が、入室しました。'.format(
                member.mention),
            colour=0x00af00)
        await text_channel.send(embed=embed, delete_after=180)
    if before.channel is not None and (
            after.channel is None or before.channel != after.channel) and str(
            before.channel.id) in voice_text_pair:
        text_channel = client.get_channel(
            voice_text_pair[str(before.channel.id)])
        embed = discord.Embed(
            title='ボイスチャンネル退出通知',
            description='{0}が、退出しました。'.format(
                member.mention),
            colour=0xaf0000)
        await text_channel.send(embed=embed, delete_after=180)
...
async def create_role_panel():
    ...
    for x in range(len(rolelist) // 20 + 1):
        ...
        embed = discord.Embed(
            title='役職パネル({0}ページ目)'.format(
                x + 1), description=content)
...
client.add_cog(Normal_Command(
    client, '普通のコマンド'))
client.add_cog(Bot_Owner_Command(
    client, 'BOTオーナー用コマンド'))
client.add_cog(Owners_Command(
    client, 'オーナーズ用コマンド'))
client.add_cog(Staff_Command(
    client, 'スタッフ用コマンド'))
client.add_cog(DM_Command(
    client, 'DM用コマンド'))
client.add_cog(Joke_Command(
    client, 'ネタコマンド'))
